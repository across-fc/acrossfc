...
P9S_ECHO = TrackedEncounter(
    name="P9S_ECHO", encounter_id=88, difficulty_id=101, partition_id=13
)
P10S_ECHO = TrackedEncounter(
    name="P10S_ECHO", encounter_id=89, difficulty_id=101, partition_id=13
)
P11S_ECHO = TrackedEncounter(
    name="P11S_ECHO", encounter_id=90, difficulty_id=101, partition_id=13
)
P12S_P1_ECHO = TrackedEncounter(
    name="P12S_P1_ECHO", encounter_id=91, difficulty_id=101, partition_id=13
)
P12S_ECHO = TrackedEncounter(
    name="P12S_ECHO", encounter_id=92, difficulty_id=101, partition_id=13
)
...
NAME_TO_TRACKED_ENCOUNTER_MAP = {
    encounter.name: encounter for encounter in TRACKED_ENCOUNTERS
}

TIER_NAME_TO_TRACKED_ENCOUNTERS_MAP: Dict[str, List[TrackedEncounter]] = {
    "ANABASEIOS": [P9S, P10S, P11S, P12S_P1, P12S],
    "ULTIMATE": [UCOB_EW, UCOB_SHB, UCOB_SB, TEA_EW, TEA_SHB, DSR_EW, TOP_EW],
}
...
CNJ = Job(
    tla="CNJ",
    name="Conjurer",
    main_category=HEALER.name,
    sub_category=REGEN_HEALER.name,
)
WHM = Job(
    tla="WHM",
    name="WhiteMage",
...
SCH = Job(
    ...
    name="Scholar",
    main_category=HEALER.name,
    sub_category=SHIELD_HEALER.name,
)
AST = Job(
    tla="AST",
    name="Astrologian",
...
SGE = Job(
    tla="SGE", name="Sage", main_category=HEALER.name, sub_category=SHIELD_HEALER.name
)
LNC = Job(tla="LNC", name="Lancer", main_category=DPS.name, sub_category=MELEE_DPS.name)
DRG = Job(
    tla="DRG", name="Dragoon", main_category=DPS.name, sub_category=MELEE_DPS.name
)
PGL = Job(
    tla="PGL", name="Pugilist", main_category=DPS.name, sub_category=MELEE_DPS.name
)
...
SAM = Job(
    tla="SAM", name="Samurai", main_category=DPS.name, sub_category=MELEE_DPS.name
)
RPR = Job(tla="RPR", name="Reaper", main_category=DPS.name, sub_category=MELEE_DPS.name)
ARC = Job(
    tla="ARC", name="Archer", main_category=DPS.name, sub_category=PRANGED_DPS.name
)
BRD = Job(tla="BRD", name="Bard", main_category=DPS.name, sub_category=PRANGED_DPS.name)
MCH = Job(
    tla="MCH", name="Machinist", main_category=DPS.name, sub_category=PRANGED_DPS.name
)
DNC = Job(
    tla="DNC", name="Dancer", main_category=DPS.name, sub_category=PRANGED_DPS.name
)
THM = Job(
    tla="THM", name="Thaumaturge", main_category=DPS.name, sub_category=CASTER_DPS.name
)
BLM = Job(
    tla="BLM", name="BlackMage", main_category=DPS.name, sub_category=CASTER_DPS.name
)
ACN = Job(
    tla="ACN", name="Arcanist", main_category=DPS.name, sub_category=CASTER_DPS.name
)
SMN = Job(
    tla="SMN", name="Summoner", main_category=DPS.name, sub_category=CASTER_DPS.name
)
RDM = Job(
    tla="RDM", name="RedMage", main_category=DPS.name, sub_category=CASTER_DPS.name
)
BLU = Job(
    tla="BLU", name="BlueMage", main_category=DPS.name, sub_category=CASTER_DPS.name
)
