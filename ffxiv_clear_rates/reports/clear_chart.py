...
def clear_chart(database: Database, encounters: List[TrackedEncounter]) -> Report:
    ...
    for encounter in encounters:
        ...
        for i in range(number_of_data_points):
            ...
            clear_order[encounter.name][i] = (
                datapoint[0], cumulative_cleared
            )

    earliest_date = sorted([
        clear_order[encounter.name][0][0]
        for encounter in encounters
    ])[0]
    latest_date = sorted([
        clear_order[encounter.name][-1][0]
        for encounter in encounters
    ])[-1]
    ...
    data_str = tabulate(table,
                        headers=[encounter.name for encounter in encounters],
                        tablefmt="tsv")
